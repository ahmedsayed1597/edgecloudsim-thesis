 ...
 * ThesisMobileDeviceManager sends tasks to the edge servers or
...
public class ThesisMobileDeviceManager extends MobileDeviceManager {
	...
	public void startEntity() {
		super.startEntity();
		simulationStartWallClockMs = System.currentTimeMillis();
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		switch (ev.getTag()) {
			case UPDATE_MM1_QUEUE_MODEL:
			{
				...
				((ThesisNetworkModel)networkModel).updateMM1QueeuModel();

				// THESIS ADDITION 2026-08-25: wall-clock checkpoint, reusing this existing
				// 5s periodic tick rather than adding new simulation overhead. Prints
				// simulated time alongside real elapsed wall-clock time so a run's log can
				// show whether per-tick cost is flat or growing over the course of a run -
				// needed to test whether the offered-load ramp-up (devices still starting
				// their first active period as late as t=50, see README) is what drives
				// runtime up disproportionately for longer simulation_time, as opposed to
				// a fixed per-run overhead. Grep RUNTIME_CHECKPOINT from run output.
				SimLogger.printLine(String.format("RUNTIME_CHECKPOINT simTime=%.1f wallClockMs=%d",
						CloudSim.clock(), System.currentTimeMillis() - simulationStartWallClockMs));
	...
	public void submitTask(TaskProperty edgeTask) {
		...
		if(delay>0){
			...
			if(selectedVM != null){
				...
				// THESIS FIX 2026-08-25: CloudSim's DatacenterBroker.bindCloudletToVm() looks
				// the cloudlet up via CloudletList.getById(), which is a linear scan
				// (list.stream().filter(...).findFirst()) over getCloudletList() from the
				// FRONT of the list. Upstream code (every *MobileDeviceManager in this
				// repository, tutorial1-4, sample_app2/3/4, DefaultMobileDeviceManager) adds
				// to this list on every task submission and never removes anything, so the
				// list grows for the entire run and each new submission scans almost the
				// full cumulative task history to find the entry it just added. That is an
				// O(n) lookup with no bound on n, giving O(n^2) total cost across a run -
				// confirmed by disassembling the compiled CloudSim classes, not by profiling.
				// Removing the entry immediately after binding keeps the list at O(1) size
				// (it only ever needs to hold the single task currently being bound) without
				// changing behavior: nothing else in this codebase reads getCloudletList().
