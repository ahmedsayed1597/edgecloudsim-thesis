...
public class MainApp {
	...
	public static final String APPLICATION_FOLDER = "thesis";

	// THESIS ADDITION: base seed for reproducible runs. Change it to draw a different
	// family of traces; keep it fixed to reproduce a published result exactly.
	public static final long BASE_SEED = 20260824L;
				...
				for(int k=0; k<SS.getSimulationScenarios().length; k++)
				{
					...
					for(int i=0; i<SS.getOrchestratorPolicies().length; i++)
					{
						...
						try
						{
							...
							// THESIS CHANGE: minimum time between events reduced 0.01 -> 0.0001 (10 ms -> 0.1 ms).
							// The upstream 10 ms quantum is the same magnitude as the URLLC-like 10 ms
							// deadline, so it would quantise away the very differences being measured.
							// 0.1 ms is 1/100th of the strict deadline and ~1/40th of the target median
							// service time, so event timing is no longer a limiting resolution.
							CloudSim.init(num_user, calendar, trace_flag, 0.0001);
	
							// Build scenario-specific factory (provides mobility, network, load, orchestrator, etc.)
							// THESIS CHANGE: derive a seed from the repetition and device count ONLY.
							// It deliberately does not depend on the policy or scenario, so every
							// strategy in a given repetition replays the identical mobility trace
							// and task arrival sequence. That makes the comparison paired.
							long seed = BASE_SEED + (long) iterationNumber * 1000000L + (long) j;

							// THESIS CHANGE: region count and decision delays are experimental
							// parameters (region_count, centralized_decision_delay,
							// decentralized_decision_delay), read from the same config file.
							ThesisConfig thesisConfig = new ThesisConfig(configFile);

							// SimUtils.RNG is public static final, so it cannot be replaced - but it
							// can be re-seeded. This covers the random draws upstream code makes
							// through SimUtils without modifying the EdgeCloudSim core.
							SimUtils.RNG.setSeed(seed);

							ScenarioFactory sampleFactory = new ThesisScenarioFactory(j,SS.getSimulationTime(), orchestratorPolicy, simScenario, seed, thesisConfig);
